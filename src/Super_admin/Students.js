...
function Students(){
    ...
    const call_period = async (prd) => {
        try{
            const res = await fetch(`/period/students/${prd}/${window.localStorage.getItem('dprt')}`,{
                method: "GET",
                headers: {
    ...
    const callAboutPage = async () => {
        try{
            ...
            if(datas.user[0].roll != 'SuperAdmin'){
                ...
                const ad = await fetch(`/super_admin/departments/students/${window.localStorage.getItem('dprt')}`,{
                    method: "GET",
                    headers: {
                        Accept: 'application/json',
                        "Content-Type": "application/json"
    ...
    return(
        <div className="iqac">
            ...
            <div style={{margin: '18px 0',textAlign: 'center',lineHeight: '30px',fontWeight: 'bolder'}}>
                ...
                <p>Department : {window.localStorage.getItem('dprt')} - Students</p>
                {
                    msg ? 
                    <>
                        {msg === 'All' ? 
