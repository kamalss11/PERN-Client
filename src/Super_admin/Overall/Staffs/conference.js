...
function Conference(){
    ...
    const call_period = async (prd,value) => {
        try{
            const res = await fetch(`/period/overall/staffs/${prd}/conference`,{
                method: "GET",
                headers: {
    ...
    const callAboutPage = async () => {
        try{
            ...
            if(datas.user[0].roll != 'SuperAdmin'){
                ...
                const ad = await fetch(`/super_admin/overall/staffs/conference`,{
                    method: "GET",
                    headers: {
                        Accept: 'application/json',
                        "Content-Type": "application/json"
        ...
        <div className="iqac">
            ...
            <div className="WordSection1">
                ...
                <div style={{textAlign: 'center'}}>
                    ...
                    <h2>Overall Conference / Seminar / Symposium / Workshop organized - Staffs Reports</h2>
                    {
                        msg ? 
                        <>
                            {msg === 'All' ? 
                ...
                <h4>3.1 Conference / Seminar / Symposium / Workshop organized ( Total - {rps ? rps.length : '0'} )</h4>
                <table>
                <tbody>
                <tr>
                    <th>S.No</th>
                ...
                {
                    rps ? rps.map((r,i)=>{
                        const {n,file,department,con_sem,title,sponsoring_agency,resource_person,venue,objective,outcome,level,total,date} = r
                        return(
                            <tr key={i}>
                                <td>{i+1}</td>
                                <td>{n ? n : '-'}</td>
                                ...
                                <td>{total ? total : '-'}</td>
                                <td>{date ? date : '-'}</td>
                                <td>{venue ? venue : '-'}</td><td>{file ? <a href={`https://localhost:3000/Uploads/${file}`}>{file}</a> : null}</td>
                            </tr>
                        )
                    }): null
                }
            ...
            <div style={{margin: '18px 0',textAlign: 'center',lineHeight: '30px',fontWeight: 'bolder'}}>
                ...
                <p>Overall Conference / Seminar / Symposium / Workshop organized - Staffs Report</p>
                {
                    msg ? 
                    <>
                        {msg === 'All' ? 
                ...
                <div className="table">
                    {
                        ...
                        ]} data={rps} title={`Conference / Seminar / Symposium / Workshop organized ( Total - ${rps ? rps.length : '0'} )`}  />
