...
function Awards(){
    ...
    const call_period = async (prd,value) => {
        try{
            const res = await fetch(`/period/overall/staffs/${prd}/awards_for_innovation`,{
                method: "GET",
                headers: {
    ...
    const callAboutPage = async () => {
        try{
            ...
            if(datas.user[0].roll != 'SuperAdmin'){
                ...
                const ad = await fetch(`/super_admin/overall/staffs/awards_for_innovation`,{
                    method: "GET",
                    headers: {
                        Accept: 'application/json',
                        "Content-Type": "application/json"
        ...
        <div className="iqac">
            ...
            <div className="WordSection1">
                ...
                <div style={{textAlign: 'center'}}>
                    ...
                    <h2>Overall Awards For Innovation - Staffs Reports</h2>
                    {
                        msg ? 
                        <>
                            {msg === 'All' ? 
                ...
                <h2>RESEARCH </h2>
                <h4>1.3 Awards for Innovation - (Total - {rps ? rps.length : '0'})</h4>
                <table>
                <tbody>
                <tr>
                    <th>S.No</th>
                ...
                {
                    rps ? rps.map((r,i)=>{
                    const {n,file,department,awardee_name,designation,award_category,title,awarding_agency,venue,level,date} = r
                        return(
                            <tr key={i}>
                                <td>{i+1}</td>
            ...
            <div style={{margin: '18px 0',textAlign: 'center',lineHeight: '30px',fontWeight: 'bolder'}}>
                ...
                <p>Overall Awards For Innovation - Staffs Report</p>
                {
                    msg ? 
                    <>
                        {msg === 'All' ? 
                ...
                <div className="table">
                    {
                        ...
                        ]} data={rps} title={`Awards for Innovation ( Total - ${rps ? rps.length : '0'} )`}/>
