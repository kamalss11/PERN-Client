...
function Paperp(){
    ...
    const call_period = async (prd,value) => {
        try{
            const res = await fetch(`/period/overall/staffs/${prd}/paper_presentation`,{
                method: "GET",
                headers: {
    ...
    const callAboutPage = async () => {
        try{
            ...
            if(datas.user[0].roll != 'SuperAdmin'){
                ...
                const ad = await fetch(`/super_admin/overall/staffs/paper_presentation`,{
                    method: "GET",
                    headers: {
                        Accept: 'application/json',
                        "Content-Type": "application/json"
        ...
        <div className="iqac">
            ...
            <div className="WordSection1">
                ...
                <div style={{textAlign: 'center'}}>
                    ...
                    <h2>Overall Paper Presentation  - Staffs Reports</h2>
                    {
                        msg ? 
                        <>
                            {msg === 'All' ? 
                ...
                <h4>5.6 Paper Presentation ( Total - {rps ? rps.length : '0'} )</h4>
                <table>
                <tbody>
                <tr>
                    <th>S.No</th>
                ...
                {
                   rps ? rps.map((r,i)=>{
                        const {n,file,department,con,title,financial_support,venue,level,date} = r
                        return(
                            <tr key={i}>
                                <td>{i+1}</td>
            ...
            <div style={{margin: '18px 0',textAlign: 'center',lineHeight: '30px',fontWeight: 'bolder'}}>
                ...
                <p>Overall Paper Presentation - Staffs Report</p>
                {
                    msg ? 
                    <>
                        {msg === 'All' ? 
                ...
                <div className="table">
                    {
                        ...
                        ]} data={rps} title={`Paper Presentation ( Total - ${rps ? rps.length : '0'} )`}  />
