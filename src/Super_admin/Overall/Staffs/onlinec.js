...
function Onlinec(){
    ...
    const call_period = async (prd,value) => {
        try{
            const res = await fetch(`/period/overall/staffs/${prd}/online_courses`,{
                method: "GET",
                headers: {
    ...
    const callAboutPage = async () => {
        try{
            ...
            if(datas.user[0].roll != 'SuperAdmin'){
                ...
                const ad = await fetch(`/super_admin/overall/staffs/online_courses`,{
                    method: "GET",
                    headers: {
                        Accept: 'application/json',
                        "Content-Type": "application/json"
        ...
        <div className="iqac">
            ...
            <div className="WordSection1">
                ...
                <div style={{textAlign: 'center'}}>
                    ...
                    <h2>Overall Teachers undergone Online-Courses - Staffs Reports</h2>
                    {
                        msg ? 
                        <>
                            {msg === 'All' ? 
                ...
                <h4>5.12 Teachers undergone Online-Courses ( Total - {rps ? rps.length : '0'} )</h4>
                <table>
                <tbody>
                <tr>
                    <th>S.No</th>
                ...
                {
                    rps ?rps.map((r,i)=>{
                        const {n,file,department,training,title,financial_support,level,duration} = r
                        return(
                            <tr key={i}>
                                <td>{i+1}</td>
            ...
            <div className="table">
                {
                    ...
                    ]} data={rps} title={`Online Courses ( Total - ${rps ? rps.length : '0'} )`} />
