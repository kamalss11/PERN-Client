...
function Patents(){
    ...
    const call_period = async (prd) => {
        try{
            const res = await fetch(`/period/overall/staffs/${prd}/patents`,{
                method: "GET",
                headers: {
    ...
    const callAboutPage = async () => {
        try{
            ...
            if(datas.user[0].roll != 'SuperAdmin'){
                ...
                const ad = await fetch(`/super_admin/overall/staffs/patents`,{
                    method: "GET",
                    headers: {
                        Accept: 'application/json',
                        "Content-Type": "application/json"
        ...
        <div className="iqac">
            ...
            <div className="WordSection1">
                ...
                <div style={{textAlign: 'center'}}>
                    ...
                    <h2>Overall Patents - Staffs Reports</h2>
                    {
                        msg ? 
                        <>
                            {msg === 'All' ? 
                ...
                {
                    rps ? rps.map((r,i)=>{
                        const {n,department,title,field,fileno,date_awarded_patent,royalty_received,providing_agency,country} = r
                        return(
                            <tr key={i}>
                                <td>{i+1}</td>
            ...
            <div style={{margin: '18px 0',textAlign: 'center',lineHeight: '30px',fontWeight: 'bolder'}}>
                ...
                <p>Overall Patents - Staffs Report</p>
                {
                    msg ? 
                    <>
                        {msg === 'All' ? 
