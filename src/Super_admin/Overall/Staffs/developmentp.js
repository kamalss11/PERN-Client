...
function Developp(){
    ...
    const call_period = async (prd,value) => {
        try{
            const res = await fetch(`/period/overall/staffs/${prd}/development_programmes`,{
                method: "GET",
                headers: {
    ...
    const callAboutPage = async () => {
        try{
            ...
            if(datas.user[0].roll != 'SuperAdmin'){
                ...
                const ad = await fetch(`/super_admin/overall/staffs/development_programmes`,{
                    method: "GET",
                    headers: {
                        Accept: 'application/json',
                        "Content-Type": "application/json"
    ...
    return(
        <div className="iqac">
            ...
            <div className="WordSection1">
                ...
                <div style={{textAlign: 'center'}}>
                    ...
                    <h2>Overall Teachers attending professional development programmes - Staffs Reports</h2>
                    {
                        msg ? 
                        <>
                            {msg === 'All' ? 
                ...
                <h4>5.11 Teachers attending professional development programmes ( Total - {rps ? rps.length : '0'} )</h4>
                <table>
                <tbody>
                <tr>
                    <th>S.No</th>
                ...
                {
                    rps ? rps.map((r,i)=>{
                        const {n,department,file,training,title,venue,financial_support,date,level} = r
                        return(
                            <tr key={i}>
                                <td>{i+1}</td>
        ...
        <div style={{margin: '18px 0',textAlign: 'center',lineHeight: '30px',fontWeight:'bolder'}}>
                ...
                <p>Overall Teachers attending professional development programmes - Staffs Report</p>
                {
                    msg ? 
                    <>
                        {msg === 'All' ? 
                ...
                <div className="table">
                        {
                            ...
                            ]} data={rps} title={`Development Programmes ( Total - ${rps ? rps.length : '0'} )`} />
                        }    
                </div>
        </div>    
        </div>
