...
function Exams(){
    ...
    const call_period = async (prd,value) => {
        try{
            const res = await fetch(`/period/overall/students/${prd}/s_exams`,{
                method: "GET",
                headers: {
    ...
    const callAboutPage = async () => {
        try{
            ...
            if(datas.user[0].roll != 'SuperAdmin'){
                ...
                const ad = await fetch(`/super_admin/overall/students/s_exams`,{
                    method: "GET",
                    headers: {
                        Accept: 'application/json',
                        "Content-Type": "application/json"
        ...
        <div className="iqac">
            ...
            <div className="WordSection1">
                ...
                <div style={{textAlign: 'center'}}>
                    ...
                    <h2>Overall Students qualifying in state/ national/ international level examinations - Students Report</h2>
                    {
                        msg ? 
                        <>
                            {msg === 'All' ? 
                ...
                <h4>1.8 Students qualifying in state/ national/ international level examinations ( Total - {placements ? placements.length : '0'} )</h4>
                <table>
                <tbody>
                <tr>
                    <th>S.No</th>
                ...
                </tr>
                {
                    placements ? placements.map((r,i)=>{
                        const {n,file,department,roll_no,exam_qualified,e_roll,date} = r
                        return(
                            <tr key={i}>
                                <td>{i+1}</td>
            ...
            <div style={{margin: '18px 0',textAlign: 'center',lineHeight: '30px',fontWeight: 'bolder'}}>
                ...
                <p>Overall Students qualifying in state/ national/ international level examinations - Students Report</p>
                {
                    msg ? 
                    <>
                        {msg === 'All' ? 
                ...
                <div className="table">
                    {
                        ...
                        ]} data={placements} title={`Students qualifying in state/ national/ international level examinations ( Total - ${placements ? placements.length : '0'} )`}  />
